...
TEST_CASE("Autogenerated add tests")
{
    ...
    SECTION("1e-05 + 1e-05 = 2e-05")
    {
         binary16 x = binary16_fromrep(0b0000000010100111); // 1e-05
         binary16 y = binary16_fromrep(0b0000000010100111); // 1e-05
         binary16 z = binary16_fromrep(0b0000000101001111); // 2e-05
         binary16 a = binary16_add(x, y);
         INFO("x = 0.0000100000 = " << binary16_tofloat(x) << " = " << binary16_torep(x));
         INFO("y = 0.0000100000 = " << binary16_tofloat(y) << " = " << binary16_torep(y));
         INFO("z = 0.0000200000 = " << binary16_tofloat(z) << " = " << binary16_torep(z));
         INFO("a                = " << binary16_tofloat(a) << " = " << binary16_torep(a));
         CHECK(a.rep == z.rep);
    }
    SECTION("1.00001 + 5.00001 = 6.000019999999999")
    {
         binary16 x = binary16_fromrep(0b0011110000000000); // 1.00001
         binary16 y = binary16_fromrep(0b0100010100000000); // 5.00001
         binary16 z = binary16_fromrep(0b0100011000000000); // 6.000019999999999
         binary16 a = binary16_add(x, y);
         INFO("x = 1.0000100000 = " << binary16_tofloat(x) << " = " << binary16_torep(x));
         INFO("y = 5.0000100000 = " << binary16_tofloat(y) << " = " << binary16_torep(y));
         INFO("z = 6.0000200000 = " << binary16_tofloat(z) << " = " << binary16_torep(z));
         INFO("a                = " << binary16_tofloat(a) << " = " << binary16_torep(a));
         CHECK(a.rep == z.rep);
    }
}
