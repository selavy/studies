...
TEST_CASE("Autogenerated add tests")
{
    ...
    {
         binary16 x = binary16_fromrep(0b0000000001010011); // 1e-05
         binary16 y = binary16_fromrep(0b0000000001010011); // 1e-05
         binary16 z = binary16_fromrep(0b0000000010100111); // 2e-05
         binary16 a = binary16_add(x, y);
         INFO("x = 0.0000100000 = " << binary16_tofloat(x) << " = " << binary16_torep(x));
         INFO("y = 0.0000100000 = " << binary16_tofloat(y) << " = " << binary16_torep(y));
         INFO("z = 0.0000200000 = " << binary16_tofloat(z) << " = " << binary16_torep(z) << " " << dump_u16(z.rep));
         INFO("a                = " << binary16_tofloat(a) << " = " << binary16_torep(a) << "     " << dump_u16(a.rep));
         CHECK(a.rep == z.rep);
    }
